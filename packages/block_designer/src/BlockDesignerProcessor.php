    ...
    public static function getFieldTypes()
    {
	    ...
	    foreach($packages as $pkg){
		    ...
		    switch($packageHandle){
			    case 'block_designer':
			    case 'block_designer_pro':
				    $directory = $pkgDirectory . 'src' . ($packageHandle != 'block_designer' ? DIRECTORY_SEPARATOR . 'BlockDesigner' : null) . DIRECTORY_SEPARATOR . 'FieldType';
				    break;
			    default:
			    	if(method_exists($pkg, 'getController')){
					    $pkgController = $pkg->getController();
					    if(method_exists($pkgController, 'getBlockDesignerDirectory') && ($bdDirectory = $pkgController->getBlockDesignerDirectory())){
						    $directory = $pkgDirectory . $bdDirectory . DIRECTORY_SEPARATOR . 'FieldType';
					    }
				    }
			    	break;
		    }
		    // Check to see if we have the BlockDesigner/FieldType directory for this package
		    if(is_dir($directory)){
			    ...
			    // We need folders of field types, within the FieldType directory
			    if (!empty($folders)) {
				    foreach ($folders as $folder) {
					    $ftDirectory = $directory . DIRECTORY_SEPARATOR . $folder . DIRECTORY_SEPARATOR;
					    $ftClassName = self::_nameSpace($folder);
					    $ftClassFile = $ftDirectory . $ftClassName . '.php';
					    // We need the file to exists, before we actually do other stuff
					    if(file_exists($ftClassFile)){
						    // Retrieve the full namespace from the file
						    $ftNamespace = '\\' . self::get_class_from_file($ftClassFile);
						    /* @var $ftClass \RamonLeenders\BlockDesigner\FieldType\FieldType
						     * we extend this "FieldType" class from its own package, it should exist of course */
						     if (trim($ftNamespace) != '' && class_exists($ftNamespace)) {
							    $ftClass = new $ftNamespace($ftDirectory, $packageHandle, $pkgDirectory, $ftClassName);
							    $ftHandle = $ftClass->getHandle();
							    // We first check if we haven't got this Field Type already in our results
							    if(!isset($results[$ftHandle])){
								    ...
								    if (($pkgVersionRequired === false || version_compare($pkgVersionRequired, $pkgBdVersion, '<=')) && ($appVersionRequired === false || version_compare($appVersionRequired, self::getConcreteVersionInstalled(), '<='))) {
									    ...
									    $icon = file_exists($iconFile) ? $iconFile : self::getPackageFolder('block_designer') . 'img' . DIRECTORY_SEPARATOR . 'icon.png';
									    $results[$ftHandle] = [
